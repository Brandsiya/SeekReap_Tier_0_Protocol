# SeekReap Tier 0 — Master Index

This index lists **all and only** the canonical, normative documents that constitute SeekReap Tier 0.

No document outside this list has Tier-0 authority.

## Canonical Tier-0 Documents

### Governance & Meta-Architecture
1. [README.md](./README.md)
2. [TIER0_OVERVIEW.md](./TIER0_OVERVIEW.md) 
3. [TIER_BOUNDARY_CONTRACT.md](./TIER_BOUNDARY_CONTRACT.md)
4. [FREEZE_ANNOUNCEMENT.md](./FREEZE_ANNOUNCEMENT.md)
5. [LICENSE](./LICENSE)

### Protocol Specification
6. [SEEKREAP_PROTOCOL_SPECIFICATION.md](./protocol/SEEKREAP_PROTOCOL_SPECIFICATION.md)
